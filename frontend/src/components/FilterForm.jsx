...
const FilterForm = ({ darkMode, onFilterChange, spotOnly }) => {
    ...
    useEffect(() => {
      // Reset budget to default value based on budget type
      setBudget(budgetType === "Hourly" ? 1 : budgetType === "Yearly" ? 1200 : 100);
    }, [budgetType]);
    ...
    return (
      <div className={`rounded-lg shadow-lg p-6 ${darkMode ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`}>
        ...
        <p className={`text-sm mb-6 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>Tell us about your workload and we'll recommend the best GPU instances for your needs.</p>
